...
3. `.claude/session-log.md` — 사용자 프롬프트 히스토리 및 수행 작업 기록
4. `.claude/prd.md` — 기능 요구사항 전체 확인 (필요 시)

## 작업 완료 후 필수 루틴 ⚠️ 순서대로 반드시 지킬 것

### 1단계 — 검증 (모두 통과해야 다음 단계 진행)
```bash
npm run typecheck   # 타입 오류 없어야 함
npm test            # 모든 테스트 통과해야 함 (신규 기능은 테스트 추가 필수)
```
보안 체크 (코드 리뷰):
- 외부 URL fetch → SSRF 방지 (도메인 whitelist, 내부IP 차단)
- 사용자 입력 → Claude 프롬프트 삽입 시 길이 제한 + JSON 구조화
- 크롬 익스텐션 메시지 → sender.id 검증 + 타입 whitelist
- DOM 조작 → textContent 사용 (innerHTML 금지)
- 모든 URL → HTTPS + 허용 도메인만 통과

### 2단계 — 기록 파일 업데이트
| 파일 | 업데이트 내용 |
|---|---|
| `.claude/sprints.md` | 완료 항목 ✅ 체크, 현재 스프린트 상태 갱신 |
| `.claude/changelog.md` | 날짜 / 작업 내용 / 변경된 파일 추가 |
| `.claude/session-log.md` | 사용자 프롬프트 + 수행 작업 항목 추가 (매 대화마다 필수) |
| `memory/project_context.md` | 아키텍처/기술 결정사항 변경 시 |
| `memory/feedback.md` | 새로운 규칙/피드백 반영 시 |

메모리 경로: `/Users/heerok/.claude/projects/-Users-heerok-Desktop----ai-Hackathon-postinghelper/memory/`

### 3단계 — Git 커밋 & 푸시 (스프린트 완료 시)
- 검증이 전부 통과된 후에만 커밋
- 커밋 메시지는 **파일별로 무엇이 왜 바뀌었는지** 상세하게 작성
- 커밋 후 반드시 push
